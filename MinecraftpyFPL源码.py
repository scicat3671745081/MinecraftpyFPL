...
# 定义支持作者的文本内容
support_author_text = """
MinecraftpyFPL 的诞生，凝聚着作者和开发人员的无数心血，制作不易！
作者和开发人员为 MinecraftpyFPL 付出了大量努力，制作过程充满艰辛！
MinecraftpyFPL 背后，是作者与开发人员的执着坚守，制作实属不易！
作者和开发人员打造 MinecraftpyFPL 历经重重困难，制作不易，敬请珍惜！
MinecraftpyFPL 来之不易，感谢作者和开发人员的辛勤付出！
制作不易，支持一下！
"""
...
def show_support_author_popup():
    root = tk.Tk()
    root.withdraw()
    result = messagebox.askyesno("支持作者", "是否支持作者并打开打赏入口？", parent=root)
    if result:
        import webbrowser
        webbrowser.open("https://b23.tv/We81j5V")
    root.destroy()
...
def handle_download_complete():
    ...
    root = tk.Tk()
    root.title("下载完成")
    tk.Label(root, text="下载已完成！").pack()
    tk.Button(root, text="确定", command=root.destroy).pack()
    root.mainloop()

def catch_error_and_log(error_type, error_code, error_message):
    # 错误日志抓取和处理
    with open(error_log_file, "a") as log_file:
        log_file.write(f"Error Type: {error_type}, Code: {error_code}, Message: {error_message}\n")

def create_ui():
    root = tk.Tk()
    root.title("MinecraftpyFPL")
    ...
    author_info = """
    作者和开发人
    bilibili：scicat科技猫新号（UID:3546648032577984）
    及 -价值5个硬币的昵称- （UID:629975293）
    打赏入口：https://b23.tv/We81j5V
    开源地址：https://github.com/scicat3671745081/MinecraftpyFPL
    基于内核：https://github.com/AEBC08/EMCLK/tree/main
    """
    tk.Label(root, text=author_info, justify="left").pack(pady=10)
...
def main():
    global launch_count
    launch_count += 1
    if launch_count >= launch_threshold:
        show_support_author_popup()
    create_ui()
